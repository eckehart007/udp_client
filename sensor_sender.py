...
class Server(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        self.logger.debug("Received %d bytes of data (%s) from %s, discarding", len(data), data, addr)
    ...
    def send_packet(self, sensor_id, value_type, max_value, interval=1):
        ...
        self.transport.sendto(bytes(packet), (IP, PORT))
...
if __name__ == "__main__":
    ...
    listen = loop.create_datagram_endpoint(Server, remote_addr=(IP, PORT))
    logger.info("transport, protocol")
    transport, protocol = loop.run_until_complete(listen)

    try:
        logger.info("loop.run_forever()")
        loop.run_forever()
